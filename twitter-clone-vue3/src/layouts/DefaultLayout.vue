<template>
  <div class="flex h-screen container mx-auto relative">
      ...
      <div class="flex flex-col items-center xl:items-start">
        ...
        <div class="flex flex-col items-start space-y-1">
          <router-link :to="route.path"
                       :class="`hover:text-primary hover:bg-blue-50 p-2 xl:px-4 xl:py-2 rounded-full cursor-pointer ${router.currentRoute.value.name == route.name ? 'text-primary' : ''}`"
    ...
    <div class="absolute bottom-20 left-12 shadow rounded-lg w-60 bg-white" v-if="showProfileDropdown"
      ...
      <button class="p-3 hover:bg-gray-50 w-full text-left text-sm" @click="onLogout">@{{ currentUser.username }} 계정에서
        로그아웃
      </button>
    </div>
    <!--  tweet modal popup  -->
